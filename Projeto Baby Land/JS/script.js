...
function cadastraBaba(){
  ...
  switch (true){
    ...
    case nome=='' || idade=='' || email=='' || senha=='' || confirmacaoSenha=='' || !termos.checked:
      mensagem.innerHTML='Alguns campos não estão preenchidos!';
      break;

    case idade>100 || idade<=0:
    ...
    case !termos.checked:
      ...
      default:
      if (babasCadastradas!=null && paisCadastrados!=null){

        for (i=0; i<babasCadastradas.length; i++){

          if (email==babasCadastradas[i].email){
        ...
        for (i=0; i<paisCadastrados.length; i++){
          ...
          if (email==paisCadastrados[i].email){
...
// previnir que a página não reinicie a hora de fazer login;
...
function logaUsuario(){
  ...
  if (usuarioResponsavel==false && usuarioBaba==true){
    ...
    if (senhaDigitada==babasCadastradas[posicaoUser].senha && emailDigitado==babasCadastradas[posicaoUser].email){
    ...
    } else if (emailDigitado == babasCadastradas[posicaoUser].email && senhaDigitada != babasCadastradas[posicaoUser].senha){

      mensagem.innerHTML = 'Senha incorreta!';
    }
  } else if (usuarioResponsavel==true && usuarioBaba==false){

    if (senhaDigitada == paisCadastrados[posicaoUser].senha && emailDigitado == paisCadastrados[posicaoUser].email){
    ...
    } else if (emailDigitado==paisCadastrados[posicaoUser].email &&senhaDigitada!=paisCadastrados[posicaoUser].senha){

      mensagem.innerHTML = 'Senha incorreta!';
    } else if (senhaDigitada == paisCadastrados[posicaoUser].senha &&emailDigitado != paisCadastrados[posicaoUser].email){
...
function editar_perfil_baba() {
  ...
  for (i = 0; validacao == false; i++) {
    if (
      ...
      if (obter_localStorage_baba[i].preferencias.length==0){

        obter_localStorage_baba[i].preferencias.push('Nenhuma preferência');
      }
